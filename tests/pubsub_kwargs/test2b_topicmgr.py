...
class TestTopicMgr0_Basic:
    ...
    def failTopicName(self, name):
        assert_raises(TopicNameError, validateName, name)
...
class TestTopicMgr1_GetOrCreate_NoDefnProv:
    ...
    def test10_NoProtoListener(self):
        ...
        def verifyNonSendable(topicObj, nameTuple, parent):
            ...
            assert_raises(TopicDefnError, topicObj.isValid, foobar)
            assert_raises(TopicDefnError, topicObj.validate, foobar)
        ...
        tName = 'Undefined'
        assert_raises(TopicNameError, topicMgr.getTopic, tName)
...
class TestTopicMgr2_GetOrCreate_DefnProv:
    ...
    def test10_DefnProvider(self):
        ...
        assert_equal(1, topicMgr.getNumDefnProviders())
        topicMgr.addDefnProvider(dp1)
        assert_equal(1, topicMgr.getNumDefnProviders())
        topicMgr.addDefnProvider(dp2)
        assert_equal(2, topicMgr.getNumDefnProviders())
        topicMgr.addDefnProvider(dp2)
        assert_equal(2, topicMgr.getNumDefnProviders())
        topicMgr.addDefnProvider(dp1)
        assert_equal(2, topicMgr.getNumDefnProviders())
        topicMgr.clearDefnProviders()
        assert_equal( 0, topicMgr.getNumDefnProviders())
        topicMgr.addDefnProvider(dp1)
        assert_equal( 1, topicMgr.getNumDefnProviders())
        topicMgr.clearDefnProviders()
    ...
    def test20_UseProvider(self):
        ...
        def testRaises(topicName, expectMsg):
            assert_raises(MessageDataSpecError, topicMgr.getOrCreateTopic, topicName)
            try:
...
\-- Topic "a2"
    ...
    def test1(self):
        ...
        buffer = py2and3.StringIO()
        printTreeDocs(rootTopic=root, width=70, fileObj=buffer)
        #print buffer.getvalue()
        assert_equal( buffer.getvalue(), self.expectedOutput )
    ...
    def test2(self):
        ...
        def exe(expectCalls, expectTopics, **kwargs):
            ...
            assert_equal( set(traverser.topics), set(expectTopics) )
            assert_equal( set(traverser.calls),  set(expectCalls) )
