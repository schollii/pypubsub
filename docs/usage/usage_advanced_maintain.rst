...
The easiest way to understand a topic tree definition is to get pubsub to output one 
...
on that setting, pubsub will look for all classes in the kwargs_topics object, 
...
``pubsub.utils.XmlTopicDefnProvider`` was contributed to pubsub by one of its 
